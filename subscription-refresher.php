	...
	if(get_user_subscription($user->id) !== 'basic') {
		...
		if(has_subscription_expired($user->id, $time)) {
			...
			if(is_subscription_active($user->id)) {
				...
				send_email($get_user_email($id), 'Notice of Subscription End', '<p>We noticed that you canceled your subscription with us. This email is a reminder that your subscription period has ended. <a href="' . site_url('pricing') .'">Renew your subscription?</a></p><br>' . $links_html);
			}
			...
			if(is_subscription_active($user->id)) {
				...
				$response = send_email($get_user_email($id), '3 Day Reminder of Subscription Renewal', '<p>Thank you for your subscription to our service! This email is a reminder that in 3 days, you will be charged automatically to continue the subscription (' . nice_stored_payment_method($user->id) . ').</p><br>' . $links_html);
			}else {
